...
**HumanEval 164 题实测**（详见 §6）：默认策略 **96.3%**（158/164），追平纯云端（95.7%），
比强制小模型先答的 84.8% 高 **+11.5 pp**。代价是代码题云端 token 占比高达 89.2% ——
因为 HumanEval 载荷中位 563 字符，85% 被 `ANALYSIS_MIN_CHARS=400` 判成分析类直接交出去了。
...
## 6. HumanEval 164 题：新路由 + flash 的实测结果

164 题，`max_tokens=2048`，`temperature=0`，pass@1（子进程跑 `check()`），
小模型 Qwen3-4B Q4_K_M（llama.cpp CUDA），云端 `deepseek-v4-flash` + 关思考。
原始数据 `evidence/offload_sweep_humaneval164.json`。

| 臂 | pass@1 | 交接率 | 云端 token 占比 | 均耗时 |
|---|---|---|---|---|
| `small` 纯本机 | 82.9%（136） | — | 0% | 1.94s |
| `cloud` 纯云端 | 95.7%（157） | — | 100% | 1.53s |
| **默认策略 relay** | **96.3%（158）** | 85.4% | 89.2% | 1.61s |

`SMALL_MAX_TOKENS` 扫描（用 `pyrodash_task=routine` 强制小模型先答，隔离出变量）：
...
1. **默认策略在代码题上拿省钱换正确率，不是两全。**
   HumanEval 载荷中位 563 字符，`ANALYSIS_MIN_CHARS=400` 把 140/164（85%）
   判成 `analysis` 直接交云端。结果：准确率追平纯云端（96.3% vs 95.7%，差 1 题在噪声内），
   但云端占比 89.2%。旧策略（所有带 tools / 长的都透传）只有 85.4%。
2. **那个路由优化值 +11.5 pp**：强制小模型先答（组 B）只有 84.8%，默认策略 96.3%。
   对短题则相反 —— 24/24 全对，只花 2543 个本机 token。
3. **`SMALL_MAX_TOKENS` 是单调的质量/成本旋钮**：越小越准、越费云端 token（见上表）。
4. **给 4B 更多预算不能提高它的正确率。** 走本机的题从 77 增到 164 道，通过率始终卡在
   85–88%；多给预算只是让它写更长。「交接后」的通过率始终 ≥「本机搞定」。

另一个意外收获：**flash + 关思考让 2048 预算就够用了**。换之前 2048 只有 83.5%
（28/164 被 `length` 截断）、要 8192 才到 95.7%；现在 2048 直接就是 95.7%。
而且均耗时从 pro 时代的数十秒降到 **1.53s**。


---

## 7. 基座替换评估：MiniCPM5-2B 值不值（不值）

既然 §6 发现「对 4B 来说输出越长越容易错」是中转的免费难度探测器，那自然要问：
**换一个小模型、把这条曲线换得更陡，是不是就能既省云端 token 又保准确率？**

实测了两个候选（MiniCPM5-2B 已实测，NVIDIA Nemotron 3 Nano 4B 结构化否决）。原始数据
`evidence/base_model_eval_minicpm5_2b.json`。同题集、同判分，只换基座：

| 基座 | 纯本机 pass@1 | 中转 pass@1 | 云端 token 占比 | 中转均耗时 |
|---|---|---|---|---|
| **Qwen3-4B-Instruct-2507**（现役） | **82.9%** | **96.3%** | **89.2%** | 1.61s |
| MiniCPM5-2B Q4_K_M（关思考） | 78.7% | 93.9% | 90.5% | **1.57s** |
| NVIDIA Nemotron 3 Nano 4B | — （未实测，结构性否决） | — | — | — |

路由分布两者完全相同（`handoff:analysis` 140 / `small` 24），差异全部来自小模型真正经手的 24 道：
**Qwen3-4B 24/24（100%）vs MiniCPM5-2B 21/24（87.5%）**。

**结论：不换。** 端到端 93.9% < 96.3%，云端占比反而从 89.2% 升到 90.5% ——
本机答错的题最后还得云端重做，省下的本地 token 被多花的云端 token 吃回去了。
它的长度曲线确实陡得多（区分度 27.2pp vs 11.3pp，2.4 倍），但这份优势**在默认路由下用不上**：
85.4% 的题被 `analysis` 直接交给云端，小模型只碰 24 道题，而在这 24 道上现役模型是满分。

### ⚠️ 比结论更重要的意外发现：开思考的模型不能当基座

MiniCPM5-2B 的 chat 模板**默认开思考**。同一批 16 题：

| | 通过 | 均耗时 | 输出 token |
|---|---|---|---|
| 开思考（模板默认） | **8/16** | 10.67s | 557 ~ 2048 |
| 关思考 | **16/16** | 1.06s | 82 ~ 223 |

开思考时全部超过 `SMALL_MAX_TOKENS=512` → 在 relay 里 100% 命中 `stop_type=='limit'`
→ 每道题都交给云端 → **中转退化成纯代理、本地零节省**。而且**顶层 `enable_thinking` 被静默忽略**，
必须放进 `chat_template_kwargs`；中转原来走 `/apply-template` + 原生 `/completion`，根本没地方传。
本次已给中转补上 `SMALL_TEMPLATE_KWARGS` 环境变量（对现役 Qwen3-4B 零副作用）。

> 📄 完整评估：`../local_relay/BASE_MODEL_EVAL.md`

*（噪声提醒：n=164 时两臂差 95%CI ±4.2pp，-2.4pp 在噪声内，严格表述是「未观察到优势」。）*
